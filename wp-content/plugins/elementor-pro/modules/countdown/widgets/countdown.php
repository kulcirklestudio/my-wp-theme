...
class Countdown extends Base_Widget
{
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'recurring' => esc_html__('Recurring Timer', 'elementor-pro'),
				],
				'default' => 'due_date',
			]
		);
	...
	protected function render()
	{
		...
		if ('evergreen' === $instance['countdown_type']) {
		...
		} else if ('recurring' === $instance['countdown_type']) {

			$timezone = new \DateTimeZone(wp_timezone_string());
			$now = new \DateTime('now', $timezone);

			$start_time = !empty($instance['recurring_start_time'])
				? new \DateTime($instance['recurring_start_time'], $timezone)
				: new \DateTime('today 00:00:00', $timezone);

			$end_time = !empty($instance['recurring_end_time'])
				? new \DateTime($instance['recurring_end_time'], $timezone)
			...
			// If we've already passed today's end time, move both to tomorrow
			if ($now > $end_time) {
		...
		$this->add_render_attribute( 'div', [
			...
			'data-date' => $due_date,
			'data-type' => $instance['countdown_type'], // add this line
		] );
